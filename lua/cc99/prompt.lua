...
PROMPT.build_system_prompt = function()
	local system_prompt_path = cc99_fs.get_prompts_dir() .. "/system.md"
	local system_prompt = cc99_fs.read_md(system_prompt_path)
	if not system_prompt then
		print("[cc99] ERROR: could not read system.md from:", system_prompt_path)
		return
	end

	local context = cc99_fs.read_cc99_md()
	if not context then
...
---@param bufnr number
---@param mark_start_line number
---@param mark_end_line number
...
PROMPT.build_user_prompt = function(bufnr, mark_start_line, mark_end_line, buf_open)
	...
	local code_lines = vim.api.nvim_buf_get_lines(0, mark_start_line, mark_end_line, false)
